...
public final class Client {
  ...
  private static List<ClientThread> initDb(String dbname, Properties props, int threadcount,
    ...
    try (final TraceScope span = tracer.newScope(CLIENT_INIT_SPAN)) {
      long opcount = operationCount(props, dotransactions);
      ...
      for (int threadid = 0; threadid < threadcount; threadid++) {
        ...
        long threadopcount = operationCountForThread(opcount, threadcount, threadid);
  ...
  static long operationCount(Properties props, boolean dotransactions) {
    if (dotransactions) {
      return Long.parseLong(props.getProperty(OPERATION_COUNT_PROPERTY, "0"));
    }
    if (props.containsKey(INSERT_COUNT_PROPERTY)) {
      return Long.parseLong(props.getProperty(INSERT_COUNT_PROPERTY, "0"));
    }
    return Long.parseLong(props.getProperty(RECORD_COUNT_PROPERTY, DEFAULT_RECORD_COUNT));
  }

  static long operationCountForThread(long opcount, int threadcount, int threadid) {
    long threadopcount = opcount / threadcount;

    // Ensure the total assigned operations still matches opcount exactly.
    if (threadid < opcount % threadcount) {
      ++threadopcount;
    }
    return threadopcount;
  }
