...
public class CoreWorkload extends Workload {
  ...
  /**
   * The name of the property for the number of request partitions.
   */
  public static final String REQUEST_PARTITION_COUNT_PROPERTY = "requestpartition.count";

  /**
   * The default number of request partitions.
   */
  public static final String REQUEST_PARTITION_COUNT_PROPERTY_DEFAULT = "1";

  /**
   * The name of the property for the current request partition index.
   */
  public static final String REQUEST_PARTITION_INDEX_PROPERTY = "requestpartition.index";

  /**
   * The default request partition index.
   */
  public static final String REQUEST_PARTITION_INDEX_PROPERTY_DEFAULT = "0";

  /**
   * The name of the property for the request partition size.
   */
  public static final String REQUEST_PARTITION_SIZE_PROPERTY = "requestpartition.size";

  /**
   * The default request partition size. When set to 0, partitions are contiguous slices.
   */
  public static final String REQUEST_PARTITION_SIZE_PROPERTY_DEFAULT = "0";
  ...
  static final class RequestPartitionRange {
    private final long lowerBound;
    private final long upperBound;
    private final String description;

    RequestPartitionRange(long lowerBound, long upperBound, String description) {
  ...
  static final class RequestDistributionState {
    private final NumberGenerator keyChooser;
    private final long lowerBound;
    private final long upperBound;
    private final String partitionDescription;

    RequestDistributionState(
        NumberGenerator keyChooser, long lowerBound, long upperBound, String partitionDescription) {
  ...
  static RequestPartitionRange requestPartitionRange(
      long lowerBound, long upperBound, int partitionCount, int partitionIndex, long partitionSize)
      throws WorkloadException {
    if (upperBound < lowerBound) {
      throw new WorkloadException("Invalid key range [" + lowerBound + ", " + upperBound + "].");
    }
    if (partitionCount < 1) {
      throw new WorkloadException(
          REQUEST_PARTITION_COUNT_PROPERTY + " (" + partitionCount + ") must be at least 1.");
    }
    if (partitionSize < 0) {
      throw new WorkloadException(
          REQUEST_PARTITION_SIZE_PROPERTY + " (" + partitionSize + ") must be non-negative.");
    }
    if (partitionCount == 1) {
      return new RequestPartitionRange(lowerBound, upperBound, "disabled");
    }
    if (partitionIndex < 0 || partitionIndex >= partitionCount) {
      throw new WorkloadException(
          REQUEST_PARTITION_INDEX_PROPERTY
              + " ("
              + partitionIndex
              + ") must be within [0, "
              + partitionCount
    ...
    long originalUpperBound = upperBound;
    long totalKeys = upperBound - lowerBound + 1;
    if (partitionSize > 0) {
      long totalShards = (totalKeys + partitionSize - 1) / partitionSize;
      if (partitionCount > totalShards) {
        throw new WorkloadException(
            REQUEST_PARTITION_COUNT_PROPERTY
                + " ("
                + partitionCount
                + ") cannot exceed shard count "
                + totalShards
                + " for "
                + REQUEST_PARTITION_SIZE_PROPERTY
                + "="
                + partitionSize
                + " over key range ["
                + lowerBound
      ...
      long shardIndex = 0;
      if (totalShards > 1) {
        shardIndex = (long) partitionIndex * (totalShards - 1) / (partitionCount - 1);
      }

      long shardLowerBound = lowerBound + shardIndex * partitionSize;
      long shardUpperBound = Math.min(shardLowerBound + partitionSize - 1, originalUpperBound);
      return new RequestPartitionRange(
          shardLowerBound,
          shardUpperBound,
          "fixed-size shard "
              + partitionIndex
    ...
    long partitionLowerBound = lowerBound + (long) partitionIndex * totalKeys / partitionCount;
    long partitionUpperBound =
        lowerBound + (long) (partitionIndex + 1) * totalKeys / partitionCount - 1;
    return new RequestPartitionRange(
        partitionLowerBound,
        partitionUpperBound,
        "contiguous partition " + partitionIndex + "/" + partitionCount);
  }

  static RequestDistributionState createRequestDistributionState(
      Properties p,
      ...
      throws WorkloadException {
    long keyrangeLowerBound = insertstart;
    long keyrangeUpperBound = insertstart + insertcount - 1;
    int partitionCount =
        Integer.parseInt(
            p.getProperty(
                REQUEST_PARTITION_COUNT_PROPERTY, REQUEST_PARTITION_COUNT_PROPERTY_DEFAULT));
    int partitionIndex =
        Integer.parseInt(
            p.getProperty(
                REQUEST_PARTITION_INDEX_PROPERTY, REQUEST_PARTITION_INDEX_PROPERTY_DEFAULT));
    long partitionSize =
        Long.parseLong(
            p.getProperty(REQUEST_PARTITION_SIZE_PROPERTY, REQUEST_PARTITION_SIZE_PROPERTY_DEFAULT));
    boolean partitionEnabled = partitionCount > 1;
    String partitionDescription = "disabled";
    NumberGenerator keychooser;
    ...
    if (requestdistrib.compareTo("uniform") == 0) {
      ...
      keyrangeLowerBound = partitionRange.lowerBound();
      keyrangeUpperBound = partitionRange.upperBound();
      partitionDescription = partitionRange.description();
      keychooser = new UniformLongGenerator(keyrangeLowerBound, keyrangeUpperBound);
    } else if (requestdistrib.compareTo("exponential") == 0) {
      if (partitionEnabled) {
        throw new WorkloadException(
            REQUEST_PARTITION_COUNT_PROPERTY
                + " is not supported with requestdistribution="
                + requestdistrib
                + ".");
      }
      ...
      keyrangeUpperBound = partitionRange.upperBound();
      partitionDescription = partitionRange.description();
      keychooser = new SequentialGenerator(keyrangeLowerBound, keyrangeUpperBound);
      ...
      keyrangeUpperBound = partitionRange.upperBound();
      partitionDescription = partitionRange.description();
      keychooser = new ScrambledZipfianGenerator(keyrangeLowerBound, keyrangeUpperBound);
      ...
      keychooser = new HotspotIntegerGenerator(
          keyrangeLowerBound, keyrangeUpperBound, hotsetfraction, hotopnfraction);
    } else {
      throw new WorkloadException("Unknown request distribution \"" + requestdistrib + "\"");
    }

    return new RequestDistributionState(
        keychooser, keyrangeLowerBound, keyrangeUpperBound, partitionDescription);
  }
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    RequestDistributionState requestDistributionState =
        createRequestDistributionState(
            p, requestdistrib, insertstart, insertcount, recordcount, transactioninsertkeysequence);
    keychooser = requestDistributionState.keyChooser();
    System.out.println(
        String.format(
            "Using request distribution '%s' keyrange [%d %d] (%s)",
            requestdistrib,
            requestDistributionState.lowerBound(),
            requestDistributionState.upperBound(),
            requestDistributionState.partitionDescription()));
