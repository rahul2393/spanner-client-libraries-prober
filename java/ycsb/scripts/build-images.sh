...
IMAGE_REPO="${IMAGE_REPO:-us-central1-docker.pkg.dev/span-cloud-testing/irahul-images/irahul-ycsb}"
DOCKERFILE="${DOCKERFILE:-java/ycsb/Dockerfile}"
...
if [[ ! -f "${DOCKERFILE}" || ! -d java/ycsb ]]; then
  echo "Run from spanner-client-libraries-prober repo root. Missing ${DOCKERFILE} or java/ycsb." >&2
  exit 1
fi
